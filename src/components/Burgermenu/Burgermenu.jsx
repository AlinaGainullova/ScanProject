...
function Burgermenu(){
    ...
    return(
        ...
        <div className={styles.burgermenu__container}>
            ...
            <Limits />
            <button className={styles.container__burger_btn} onClick={menuClickHandler}><Burger /></button>
        </div>)
        ||
        (<div className={styles.burgermenu__menu}>
            ...
            <nav className={styles.menu__select}>
                <NavLink to="/" className={styles.menu__link}>Главная</NavLink>
                <NavLink to="/" className={styles.menu__link}>Тарифы</NavLink>
                <NavLink to="/" className={styles.menu__link}>FAQ</NavLink>
            </nav>
            <button className={styles.menu__btn1}>Зарегистрироваться</button>
            {!isAuth &&
            (<button className={styles.menu__btn2}><Link to="/auth" onClick={menuClickHandler} className={styles.btn2__link}>Войти</Link></button>)
            }
            {isAuth &&
            (<button className={styles.menu__btn2} onClick={() => dispatch(setAuth(false))}>Выйти</button>)
            }
        </div>)
        }    
        </>
