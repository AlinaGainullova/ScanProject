...
export default function DateRangeInputs({ selectedStartDate, selectedEndDate, setSelectedStartDate, setSelectedEndDate, isError, setError }) {
    ...
    return (
        <>
            {getDatePicker(isError, startDateTxt, startDate, selectedStartDate, onSelectStartDate, "startDate", open, setOpen)}
            {getDatePicker(isError, endDateTxt, endDate, selectedEndDate, onSelectEndDate, "endDate")}
        </>
    );
}

function getDatePicker(isError, value, startText, selectedDate, onSelectDate, dayPickerId, open, setOpen) {
    return (
        <Disclosure>
            <div className={styles.disclosure__container}>
                <Disclosure.Button onChange={setOpen} className={isError ? styles.error : styles.container__buttons}>
                    {({ open }) => (
                        <>
                ...
                <Disclosure.Panel className={styles.container__calendar}>
                    {({ close }) => (
